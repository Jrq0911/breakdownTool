...
def prepare_asset(target_dir: Path, source: str, branch: str = None):
    """执行 Git Clone (主线程调用，用于 SDK)"""
    if not source:
        log_msg(f"  [跳过] 未配置地址", is_error=True)
        return False
    
    # 如果目标目录已存在，先删除，防止残留文件干扰
...
def process_kit_task(row, local_repo: Path, IS_LOCAL_MODE: bool, output_root: Path, branch: str):
    ...
    if IS_LOCAL_MODE:
        ...
        try:
            ...
            # 本地复制时显式排除 .git 目录
            shutil.copytree(local_comp_path, target_comp_dir, dirs_exist_ok=True, ignore=shutil.ignore_patterns('.git'))
        ...
        for b in branches_to_try:
            # 使用 --depth 1 不会拉取历史，从根本上避免庞大的 .git 目录
...
def zip_kit_task(kit_dir: Path, output_dir: Path):
    """按 Kit 打包为 zip，统一存放到平级的 output 目录中"""
    if not kit_dir.exists() or not any(kit_dir.iterdir()):
        return (kit_dir.name, "SKIP", "目录为空或不存在")
    
    # 指定 zip 生成到平级的 output 目录下
    zip_path = output_dir / f"{kit_dir.name}.zip"
    
    try:
        # 严格过滤掉 .git 隐藏文件夹和 .zip 文件
        files_to_zip = [
            f for f in kit_dir.rglob('*') 
            if f.is_file() 
            and f.suffix.lower() != '.zip'
            and '.git' not in f.parts  # 确保路径中不包含任何层级的 .git 目录
        ]
        ...
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            for file_path in files_to_zip:
                try:
                    # 打包内的相对路径以 Kit 文件夹为基准 (例如：Multimedia_Kit/Player/xxx.js)
...
def main():
    ...
    # 创建统一的打包输出目录
    zip_output_dir = output_root / "output"
    zip_output_dir.mkdir(parents=True, exist_ok=True)

    # ==================== 阶段0: 初始化骨架 ====================
    log_msg("\n--- 阶段0: 初始化标准目录骨架 ---")
    for base in [oh_root, hmos_root]:
        # 【修改点】：完全移除了 Docs 文件夹的创建
    ...
    if IS_LOCAL_MODE:
        ...
        for target_dir, rel_path in sdk_local_map.items():
            ...
            if src_path.exists():
                shutil.copytree(src_path, target_dir, dirs_exist_ok=True, ignore=shutil.ignore_patterns('.git'))
                log_msg(f"  [成功] 从本地复制")
            else:
                log_msg(f"  [警告] 本地路径不存在: {src_path}", is_error=True)
    else:
        log_msg("  检测到下载模式，从固定地址 Clone...")
        sdk_url_map = {
    ...
    # ==================== 阶段3: 按 Kit 多线程打包 ====================
    log_msg("\n--- 阶段3: 按 Kit 归档打包 (统一存放到 output 目录) ---")
    kit_dirs = []
    for base in [oh_root, hmos_root]:
        for kit_dir in base.iterdir():
            if kit_dir.is_dir() and kit_dir.name not in ["SDK"]: # 排除 SDK 目录
                kit_dirs.append(kit_dir)
                
    zip_results = []
    if kit_dirs:
        with ThreadPoolExecutor(max_workers=args.threads) as executor:
            futures = [executor.submit(zip_kit_task, kd, zip_output_dir) for kd in kit_dirs]
            for future in as_completed(futures):
                zip_results.append(future.result())
                
        for name, status, err in zip_results:
            if status == "SUCCESS":
                log_msg(f"  [打包成功] output 目录下已生成 {name}.zip")
            elif status == "ERROR":
    ...
    log_msg("\n" + "="*60)
    log_msg("✅ 全部任务执行完毕！详情请查看 result.log")
    log_msg("="*60 + "\n")
